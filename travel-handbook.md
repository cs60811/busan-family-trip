...
| 18:15 | 出關 / 簡單補給 | 30 分 | 買水、整理行李；順路到 1F 4 號門旁的釜山 PASS 自助領卡機領卡（掃訂單 QR → 取卡 → 再掃卡背 QR；領卡不會啟用有效期）。交通卡不用另買——PASS 實體卡本身就是可加值的交通卡，到地鐵站加值機用現金儲值即可。換錢在機場輕軌站的換錢所換就夠，不用專程跑西面／南浦 |
